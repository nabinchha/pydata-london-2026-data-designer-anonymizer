...
Online mode (makes one tiny LLM call + one Anonymizer call to verify endpoints):
...
def check_seed_data() -> None:
    ...
    if missing_seed:
        warn(
            "Seed data missing. Both parquets should be checked in -- try "
...
def check_online() -> list[str]:
    ...
    try:
        ...
        ok("Anonymizer endpoints reachable (default hosted GLiNER-PII + LLM on NVIDIA Build)")
    except Exception as exc:
        warn(
            f"Anonymizer check skipped or failed: {exc}. "
            f"This is not blocking for the workshop -- by default, Anonymizer uses "
            f"NVIDIA Build hosted endpoints for both GLiNER-PII and the LLM, so "
            f"it just needs your NVIDIA_API_KEY when you first use it."
        )
    return failures
